...
SELECT
    --DLHK
    MD5(TRIM(COALESCE(U.ID,'00000000000000000000000000000000')) ) AS K_USER_BK
    ,MD5(TRIM(COALESCE(U.ACCOUNTID,'00000000000000000000000000000000')) ) AS K_ACCOUNT_BK
    ,MD5(TRIM(COALESCE(U.CONTACTID,'00000000000000000000000000000000')) ) AS K_CONTACT_BK
    ,MD5(TRIM(COALESCE(U.INDIVIDUALID,'00000000000000000000000000000000')) ) AS K_INDIVIDUAL_BK
    ,MD5(TRIM(COALESCE(U.MANAGERID,'00000000000000000000000000000000')) ) AS K_MANAGER_BK
    ,MD5(TRIM(COALESCE(U.PROFILEID,'00000000000000000000000000000000')) ) AS K_PROFILE_BK 
    ,MD5(TRIM(COALESCE(U.USERROLEID,'00000000000000000000000000000000')) ) AS K_USER_ROLE_BK
...
FROM
    ...
    LEFT JOIN user_profile UP ON UP.ID = UR.PROFILEID
)
